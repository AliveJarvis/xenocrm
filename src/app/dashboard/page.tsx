import { getServerSession } from 'next-auth';
   ...
   async function getDashboardMetrics(userId: string) {
     ...
     const [customerCount, campaignCount, recentLogs] = await Promise.all([
       ...
       CommunicationLog.find({ customerId: { $in: await Campaign.find({ userId }).distinct('customers') } })
         .sort({ timestamp: -1 })
         .limit(5)
   ...
   export default async function Dashboard() {
     ...
     if (!session?.user) {
       return (
         <div className="text-center py-16">
           <h1 className="text-2xl font-bold text-red-600">Please sign in to access the dashboard</h1>
     ...
     const { customerCount, campaignCount, recentLogs } = await getDashboardMetrics(session.user.id);
         ...
         <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
           ...
           <Card>
             ...
             <CardContent>
               ...
               <p className="text-xs text-muted-foreground">Campaigns created by you</p>
             </CardContent>
           </Card>
           ...
           <Card>
             ...
             <CardContent>
               ...
               <p className="text-xs text-muted-foreground">Recent campaign messages</p>
             </CardContent>
           </Card>
         </div>
               ...
               <ul className="space-y-4">
                 {recentLogs.map((log: any, index: number) => (
                   <li key={index} className="flex justify-between items-center">
                     <span className="text-sm">{log.message.slice(0, 50)}...</span>
                     <span
